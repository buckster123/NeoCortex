...
Usage:
    ...
    cortex post <message>           # Store a memory
    cortex session leave <summary>  # Save session note
    cortex session get              # Get recent sessions
...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="cortex",
        description="Neo-Cortex: Unified Memory System for AI Agents",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    cortex stats
    cortex search "how to deploy"
    cortex post "Learning about memory systems" --visibility shared
    cortex session leave "Session summary" --unfinished "Task 1" "Task 2"
    cortex health
    cortex export
        """,
    )
    ...
    parser.add_argument("--agent", default="CLAUDE", help="Agent ID (default: CLAUDE)")
    ...
    search_parser.add_argument("--visibility", choices=["all", "shared", "private"], default="all")
    search_parser.add_argument("--json", action="store_true")

    # === post ===
    post_parser = subparsers.add_parser("post", help="Store a memory")
    post_parser.add_argument("content", help="Message content")
    post_parser.add_argument("--visibility", choices=["shared", "private", "thread"], default="shared")
    post_parser.add_argument("--type", choices=["dialogue", "fact", "observation", "question", "discovery"], default="dialogue")
    ...
    session_leave = session_sub.add_parser("leave", help="Save a session note")
    session_leave.add_argument("summary", help="Session summary")
    session_leave.add_argument("--discoveries", nargs="*", help="Key discoveries")
    session_leave.add_argument("--unfinished", nargs="*", help="Unfinished business")
    session_leave.add_argument("--priority", choices=["HIGH", "MEDIUM", "LOW"], default="MEDIUM")
    session_leave.add_argument("--type", choices=["orientation", "technical", "emotional", "task"], default="orientation")

    session_get = session_sub.add_parser("get", help="Get recent sessions")
    session_get.add_argument("--hours", type=int, default=168)
    session_get.add_argument("--limit", type=int, default=5)
    session_get.add_argument("--priority", choices=["HIGH", "MEDIUM", "LOW"])
    session_get.add_argument("--json", action="store_true")
    ...
    health_parser.add_argument("--duplicates", action="store_true")
    health_parser.add_argument("--collection", help="Specific collection")
...
def cmd_post(cortex: CortexEngine, args) -> int:
    ...
    if result["success"]:
        print(f"Stored in {result['visibility']} memory: {result['id']}")
        return 0
    else:
        print(f"Error: {result.get('error')}")
        return 1
...
def cmd_session(cortex: CortexEngine, args) -> int:
    if args.session_action == "leave":
        ...
        if result["success"]:
            print(f"Session note saved: {result['session_id']}")
            print(f"Priority: {result['priority']}, Type: {result['session_type']}")
            return 0
        else:
            print(f"Error: {result.get('error')}")
            return 1
        ...
        print(f"Session Notes (last {args.hours} hours)")
        print("-" * 60)
        ...
        print("Use: cortex session leave|get")
        return 1


def cmd_health(cortex: CortexEngine, args) -> int:
    if args.stale is not None:
        coll = args.collection or "cortex_shared"
...
def cmd_export(cortex: CortexEngine, args) -> int:
    core = cortex.export_memory_core(agent_id=args.agent, collections=args.collections)
